...
register_activation_hook(__FILE__, 'justinpickard_darksky_weather_install');

function justinpickard_darksky_weather_install(){
    global $wp_version;
...
register_deactivation_hook(__FILE__, 'justinpickard_darksky_weather_deactivate');

function justinpickard_darksky_weather_deactivate(){
    //do something when deactivating
}

add_shortcode ( 'darksky', 'get_darksky_json_shortcode' );
 ...
 * @return string
 */
function get_darksky_json_shortcode($atts, $content = null) {

	//hard coded URL for MY API 
	$url = 'https://api.darksky.net/forecast/447e87c0a41488dba573f4440afcd8f5/37.8267,-122.4233';

	$data = get_darksky_json($url);
	...
	$output .= '<br>';
	$output .= "Temperature: " . round($data['currently']['temperature']);
	$output .= '<br>';
	$output .= "Visibility: " . $data['currently']['visibility'];
	$output .= '<br>';
	$output .= "Wind Speed: " . $data['currently']['windSpeed'];
	$output .= '<br>';
	$output .= "UV-Index: " . $data['currently']['uvIndex'];
	$output .= '<br>';
	$output .= "Humidity: " . $data['currently']['humidity'];
	$output .= '<br>';
	$output .= "Cloud Cover: " . $data['currently']['cloudCover'];
	$output .= '<br>';
	$output .= "Summary: " . $data['daily']['summary'];
	$output .= '<br>';
	$output .= '<br>';
...
add_shortcode( 'timezone', 'get_google_json_shortcode' );
 ...
 * @return string
 */
function get_google_json_shortcode($atts, $content = null) {

	//hard coded URL for MY API 
	$url ='https://maps.googleapis.com/maps/api/timezone/json?location=34.9802778,-101.9183333&timestamp=1458000000&key=AIzaSyBQClnAkLLyMDNFuSf3Et5j8d5CxvKgplM';

	$data = get_google_json($url);

	$output = '';
	$output .= '<br>';
	$output = 'Google Timezone API Call';
	$output .= '<br>';
	$output .= "dstOffset: " . $data['dstOffset'];
	$output .= '<br>';
	$output .= "rawOffset: " . $data['rawOffset'];
	$output .= '<br>';
	$output .= "Status: " . $data['status'];
	$output .= '<br>';
	$output .= "TimeZone ID: " . $data['timeZoneId'];
	$output .= '<br>';
	$output .= "TimeZone Name: " . $data['timeZoneName'];
	return $output;
...
add_action( 'widgets_init', 'justinpickard_darksky_weather_create_widgets' );

function justinpickard_darksky_weather_create_widgets() {
	register_widget( 'Justinpickard_DarkySky_Weather' );
}

class Justinpickard_DarkySky_Weather extends WP_Widget {
	// Construction function
	function __construct () {
		parent::__construct( 'Justinpickard_DarkySky_Weather', 'DarkSky Weather',
			array( 'description' =>
